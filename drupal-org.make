; Open Hotel make file
core = "7.x"
api = "2"
; comment this in to use in local development
; includes[] = drupal-org-core.make

; Modules

projects[backup_migrate][subdir] = contrib
projects[backup_migrate][version] = 2.x

projects[admin_menu][subdir] = contrib
projects[admin_menu][version] = 3.x

projects[ctools][version] = 1.x
projects[ctools][subdir] = contrib
projects[ctools][patch][1925018] = http://drupal.org/files/ctools-1925018-61.patch

projects[diff][subdir] = contrib
projects[diff][version] = 3.x

projects[extlink][subdir] = contrib
projects[extlink][version] = 1.x

projects[link][subdir] = contrib
projects[link][version] = 1.x

projects[entity][subdir] = contrib
projects[entity][version] = 1.x
projects[entityreference][subdir] = contrib
projects[entityreference][version] = 1.x
projects[file_entity][subdir] = contrib
projects[file_entity][version] = 2.x

projects[libraries][version] = 2.x
projects[libraries][subdir] = contrib

projects[pathauto][version] = 1.x
projects[pathauto][subdir] = contrib
projects[pathauto][patch][936222] = http://drupal.org/files/pathauto-persist-936222-130-pathauto-state.patch

projects[commerce][version] = "1.x"
projects[commerce][subdir] = "contrib"

projects[commerce_google_analytics][version] = "1.x"
projects[commerce_google_analytics][subdir] = "contrib"

projects[field_redirection][version] = "2.x"
projects[field_redirection][subdir] = "contrib"

projects[ccl][version] = "1.x"
projects[ccl][subdir] = "contrib"

projects[date][version] = "2.x"
projects[date][subdir] = "contrib"

projects[profiler_builder][version] = "1.x"
projects[profiler_builder][subdir] = "contrib"

projects[features][version] = "2.x"
projects[features][subdir] = "contrib"

projects[features_extra][version] = "1.x"
projects[features_extra][subdir] = "contrib"

projects[ftools][version] = "1.x"
projects[ftools][subdir] = "contrib"

projects[addressfield][version] = "1.x"
projects[addressfield][subdir] = "contrib"

projects[flexslider][version] = "2.x"
projects[flexslider][subdir] = "contrib"

projects[flexslider_views_slideshow][version] = 2.x
projects[flexslider_views_slideshow][subdir] = contrib

projects[cs_adaptive_image][version] = "1.x"
projects[cs_adaptive_image][subdir] = "contrib"

; projects[l10n_update][version] = "1.x"
; projects[l10n_update][subdir] = "contrib"

projects[i18n][version] = "1.x"
projects[i18n][subdir] = "contrib"

projects[languageicons][version] = "1.x"
projects[languageicons][subdir] = "contrib"

; projects[rules][version] = "2.x"
; projects[rules][subdir] = "contrib"

projects[colorbox][version] = "2.x"
projects[colorbox][subdir] = "contrib"

; projects[module_filter][version] = "1.x"
; projects[module_filter][subdir] = "contrib"

; projects[strongarm][version] = "2.x"
; projects[strongarm][subdir] = "contrib"

; projects[token][version] = "1.x"
; projects[token][subdir] = "contrib"

projects[transliteration][version] = "3.x"
projects[transliteration][subdir] = "contrib"

projects[rooms][version] = "1.x"
projects[rooms][subdir] = "contrib"

projects[rules][version] = "2.x"
projects[rules][subdir] = "contrib"

; projects[google_analytics][version] = "1.x"
; projects[google_analytics][subdir] = "contrib"

projects[omega_tools][version] = "3.x"
projects[omega_tools][subdir] = "contrib"

projects[variable][version] = "2.x"
projects[variable][subdir] = "contrib"

projects[draggableviews][version] = "2.x"
projects[draggableviews][subdir] = "contrib"

projects[views][version] = "3.x"
projects[views][subdir] = "contrib"

; projects[views_slideshow][version] = "3.x"
; projects[views_slideshow][subdir] = "contrib"

; projects[webform][version] = "3.x"
; projects[webform][subdir] = "contrib"

projects[context][version] = "3.x"
projects[context][subdir] = "contrib"

; Themes
projects[omega][type] = "theme"
projects[omega][version] = "3.x"
projects[omega][subdir] = "contrib"

; Libraries
libraries[flexslider][directory_name] = "flexslider"
libraries[flexslider][download][type] = git
libraries[flexslider][destination] = "libraries"
libraries[flexslider][download][url] = https://github.com/woothemes/FlexSlider.git

libraries[colorbox][download][type] = git
libraries[colorbox][download][url] = https://github.com/jackmoore/colorbox.git
libraries[colorbox][download][tag] = 1.4.4
libraries[colorbox][destination] = "libraries"

libraries[jquery.cycle][directory_name] = "jquery.cycle"
libraries[jquery.cycle][type] = "library"
libraries[jquery.cycle][destination] = "libraries"
libraries[jquery.cycle][download][type] = "get"
libraries[jquery.cycle][download][url] = "https://raw.github.com/malsup/cycle/master/jquery.cycle.all.js"

projects[coffee][subdir] = contrib
projects[coffee][version] = 2.x

projects[context][subdir] = contrib
projects[context][version] = 3.x

projects[checklistapi][subdir] = contrib
projects[checklistapi][version] = 1.x

projects[dhtml_menu][subdir] = contrib
projects[dhtml_menu][version] = 1.x

projects[feeds][subdir] = contrib
projects[feeds][version] = 2.x

projects[feeds_xpathparser][subdir] = contrib
projects[feeds_xpathparser][version] = 1.x

projects[globalredirect][subdir] = contrib
projects[globalredirect][version] = 1.x

projects[job_scheduler][subdir] = contrib
projects[job_scheduler][version] = 2.x

projects[jquerymenu][subdir] = contrib
projects[jquerymenu][version] = 4.x

projects[jw_player][subdir] = contrib
projects[jw_player][version] = 1.x

projects[menu_editor][subdir] = contrib
; missing version - add version.

projects[menu_firstchild][subdir] = contrib
projects[menu_firstchild][version] = 1.x

projects[menu_position][subdir] = contrib
projects[menu_position][version] = 1.x

projects[metatag][subdir] = contrib
projects[metatag][version] = 1.x

projects[migrate][subdir] = contrib
; missing version - add version.

projects[migrate_extras][subdir] = contrib
projects[migrate_extras][version] = 2.x

projects[nice_menus][subdir] = contrib
projects[nice_menus][version] = 2.x

projects[node_convert][subdir] = contrib
projects[node_convert][version] = 1.x

projects[redirect][patch][] = https://drupal.org/files/redirect-reenable_global_redirections-905914-121.patch
projects[redirect][subdir] = contrib
projects[redirect][version] = 1.x

    projects[path_redirect_import][subdir] = contrib
    projects[path_redirect_import][version] = 1.x

projects[search404][subdir] = contrib
projects[search404][version] = 1.x

projects[seo_checker][subdir] = contrib
projects[seo_checker][version] = 1.x

projects[seo_checklist][subdir] = contrib
projects[seo_checklist][version] = 4.x

projects[site_verify][subdir] = contrib
projects[site_verify][version] = 1.x

projects[strongarm][subdir] = contrib
projects[strongarm][version] = 2.x

projects[superfish][subdir] = contrib
projects[superfish][version] = 1.x

; projects[wordpress_migrate][subdir] = contrib
; projects[wordpress_migrate][version] = 2.x

projects[xmlsitemap][subdir] = contrib
projects[xmlsitemap][version] = 2.x

projects[views_slideshow][subdir] = contrib
projects[views_slideshow][version] = 3.x

    projects[vscc][subdir] = contrib
    projects[vscc][version] = 1.x

projects[nodequeue][subdir] = contrib
projects[nodequeue][version] = 2.x

projects[auto_nodequeue][subdir] = contrib
projects[auto_nodequeue][version] = 1.x

; projects[disqus][subdir] = contrib
; projects[disqus][version] = 1.x

; projects[menu_icons][subdir] = contrib
; projects[menu_icons][version] = 3.x

; projects[stage_file_proxy][subdir] = contrib
; projects[stage_file_proxy][version] = 1.x

projects[robotstxt][subdir] = contrib
projects[robotstxt][version] = 1.x

projects[js_injector][subdir] = contrib
projects[js_injector][version] = 2.x

projects[css_injector][subdir] = contrib
projects[css_injector][version] = 1.x

projects[google_analytics][subdir] = contrib
projects[google_analytics][version] = 1.x

projects[special_menu_items][subdir] = contrib
projects[special_menu_items][version] = 2.x

; Google Authorship

projects[google_authorship][subdir] = contrib
projects[google_authorship][version] = 2.x

; Share This - social media

; projects[sharethis][subdir] = contrib
; projects[sharethis][version] = 2.x

; Metatags Quick - adds meta tags support for site pages

projects[metatags_quick][subdir] = contrib
projects[metatags_quick][version] = 2.x

projects[security_review][subdir] = contrib
projects[security_review][version] = 1.x

projects[webform][subdir] = contrib
projects[webform][version] = 4.x

projects[menu_attributes][subdir] = contrib
projects[menu_attributes][version] = 1.x

; nice module crops images
; projects[manualcrop][subdir] = contrib
; projects[manualcrop][version] = 1.x

; force newer version of module_filter
projects[module_filter][subdir] = contrib
projects[module_filter][version] = 2.x

; projects[content_experiments][subdir] = contrib
; projects[content_experiments][version] = 1.x

projects[simple_gmap][subdir] = contrib
projects[simple_gmap][version] = 1.x

; The Panopoly Foundation

; this project (Panopoly Core) has a makefile inside, replace with makefile's content and update properly the inside projects to their latest versions. see example of makefile recrusion https://drupal.org/files/oa2-fix_makefile_recursion-1989188-5.patch
; projects[panopoly_core][subdir] = panopoly
; projects[panopoly_core][version] = 1.x

    ; Panopoly Core Makefile

    ; Panels and Chaos Tools Magic

    ; projects[panels][version] = 3.x
    ; projects[panels][subdir] = contrib
    ; projects[panels][download][type] = git
    ; projects[panels][download][revision] = 2bb470e
    ; projects[panels][download][branch] = 7.x-3.x

        projects[panels][subdir] = contrib
        projects[panels][version] = 3.x

    projects[panels_breadcrumbs][version] = 2.x
    projects[panels_breadcrumbs][subdir] = contrib

    projects[panelizer][version] = 3.x
    projects[panelizer][subdir] = contrib

    ; projects[fieldable_panels_panes][version] = 1.x
    ; projects[fieldable_panels_panes][subdir] = contrib
    ; projects[fieldable_panels_panes][download][type] = git
    ; projects[fieldable_panels_panes][download][revision] = 3b9c8b6
    ; projects[fieldable_panels_panes][download][branch] = 7.x-1.x

        projects[fieldable_panels_panes][subdir] = contrib
        projects[fieldable_panels_panes][version] = 1.x

    projects[pm_existing_pages][version] = 1.x
    projects[pm_existing_pages][subdir] = contrib

    ; projects[fape][version] = 1.x
    ; projects[fape][subdir] = contrib
    ; projects[fape][download][type] = git
    ; projects[fape][download][revision] = 1143ee2
    ; projects[fape][download][branch] = 7.x-1.x

        projects[fape][subdir] = contrib
        projects[fape][version] = 1.x

    ; Views Magic

    projects[views][version] = 3.x
    projects[views][subdir] = contrib

    projects[views_autocomplete_filters][version] = 1.x
    projects[views_autocomplete_filters][subdir] = contrib

    projects[views_bulk_operations][version] = 3.x
    projects[views_bulk_operations][subdir] = contrib

    ; The Usual Suspects

    projects[token][version] = 1.x
    projects[token][subdir] = contrib

    projects[field_group][version] = 1.x
    projects[field_group][subdir] = contrib

    projects[tablefield][version] = 2.x
    projects[tablefield][subdir] = contrib

    ; projects[simple_gmap][version] = 1.x
    ; projects[simple_gmap][subdir] = contrib

    ; Panopoly - Contrib - Widgets

    projects[menu_block][version] = 2.x
    projects[menu_block][subdir] = contrib

    ; Panopoly - Contrib - Files & Media

    ; projects[media][version] = 2.x
    ; projects[media][subdir] = contrib
    ; projects[media][download][type] = git
    ; projects[media][download][revision] = 2319170
    ; projects[media][download][branch] = 7.x-2.x

        projects[media][subdir] = contrib
        projects[media][version] = 2.x

    ; projects[media_youtube][version] = 2.x
    ; projects[media_youtube][subdir] = contrib
        projects[media_youtube][version] = 2.x
        projects[media_youtube][subdir] = contrib

    projects[media_vimeo][version] = 2.x
    projects[media_vimeo][subdir] = contrib

; end of Panopoly Widgets Makefile

; Themes

projects[danland][subdir] = contrib
projects[danland][type] = theme
projects[danland][version] = 1.x

projects[kalatheme][subdir] = contrib
projects[kalatheme][type] = theme
projects[kalatheme][version] = 1.x

projects[kanji][subdir] = contrib
projects[kanji][type] = theme
projects[kanji][version] = 3.x

